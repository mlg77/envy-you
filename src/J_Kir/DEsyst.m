...
dy(ind.K_p     ) = AC(flu.J_BK_k) / (VR_pa*state(ind.R_k)) + (SMC(flu.J_KIR_i))/(VR_ps)-(state(ind.K_p)-3000);     % uM s-1
...
dy(ind.k_k) = (SMC(flu.al)+SMC(flu.bet))*(SMC(flu.al)/(SMC(flu.al)+SMC(flu.bet))-state(ind.k_k));

% %astr. DE for Ca2+ (Hannah)
%dy(ind.ck)=B_cyt*(AC(flu.J_ip3)-AC(flu.J_pump)+AC(flu.J_ERleak)); % FARR astrocytic calcium concentration
dy(ind.ck)=AC(flu.B_cyt)*(AC(flu.J_ip3)-AC(flu.J_pump)+AC(flu.J_ERleak)+AC(flu.J_TRPV_k)); %L&E astrocytic calcium concentration
%
...
dy(ind.Ca_i)    = (SMC(flu.Ca_coup_i) + SMC(flu.rho_i) * (SMC(flu.J_CICR_i) + SMC(flu.J_IP3_i) + SMC(flu.J_leak_i) - SMC(flu.J_SRuptake_i) - SMC(flu.J_extrusion_i)...
    - SMC(flu.J_Ca) - SMC(flu.J_VOCC_i) + SMC(flu.J_NaCa_i) + 0.1*SMC(flu.J_stretch_i)));
% - SMC(flu.J_Ca)
dy(ind.s_i) 	= - SMC(flu.J_CICR_i) - SMC(flu.J_leak_i) + SMC(flu.J_SRuptake_i) ;
dy(ind.v_i)     = SMC(flu.v_coup_i) + gam * ( - SMC(flu.J_Ca)-SMC(flu.J_NaK_i) - SMC(flu.J_Cl_i) - 2*SMC(flu.J_VOCC_i) - SMC(flu.J_NaCa_i) - SMC(flu.J_K_i) - SMC(flu.J_stretch_i) - SMC(flu.J_KIR_i));
%-SMC(flu.J_Ca)
