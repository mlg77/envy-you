...
% Astrocyte
dy(ind.R_k_star) = (T_c/Rk_0)*( L_p * (AC(flu.Na_k) + AC(flu.K_k) + AC(flu.Cl_k) + AC(flu.HCO3_k) - AC(flu.Na_s) - AC(flu.K_s) - AC(flu.Cl_s) - AC(flu.HCO3_s) + X_k / (state(ind.R_k_star)*Rk_0))); % m s-1
dy(ind.N_Na_k_star) = (T_c/(Rk_0*K_0))*( -AC(flu.J_Na_k) - 3 * AC(flu.J_NaK_k) + AC(flu.J_NKCC1_k) + AC(flu.J_NBC_k )); % uMm s-1
...
% dy(ind.Ca_k_star)= (T_c/Ca_0)*(         AC(flu.B_cyt)*(AC(flu.J_ip3)-AC(flu.J_pump)+AC(flu.J_ERleak))); %L&E astrocytic calcium concentration
% dy(ind.s_k_star)= (T_c/Ca_0)*(         -1/VR_ERcyt*dy(ind.Ca_k_star)); % ER calcium concentration   !!!!!!!!!!!!!!!!!!!!!
...
% dy(ind.I_k_star)= (T_c/Ca_0)*(         r_h*AC(flu.G_pr)-k_deg*AC(flu.ik)); %IP3 concentration

% if AC(flu.ck)> ck_min
% dy(ind.eet_k_star)= (T_c/Ca_0)*( V_eet*(AC(flu.ck)-ck_min)-k_eet*AC(flu.eetk)); % L&E EET concentration
% else
% dy(ind.eet_k_star)= (T_c/Ca_0)*( -k_eet*AC(flu.eetk)); %L&E
% end

% Smooth muscle cell
dy(ind.Ca_i_star) = (T_c/Ca_0)*( SMC(flu.Ca_coup_i) + SMC(flu.rho_i) * (SMC(flu.J_CICR_i) + SMC(flu.J_IP3_i) + SMC(flu.J_leak_i) - SMC(flu.J_SRuptake_i) - SMC(flu.J_extrusion_i) - SMC(flu.J_VOCC_i) + SMC(flu.J_NaCa_i) + 0.1*SMC(flu.J_stretch_i)));
dy(ind.s_i_star) = (T_c/Ca_0)*( -SMC(flu.J_CICR_i) - SMC(flu.J_leak_i) + SMC(flu.J_SRuptake_i) );
dy(ind.v_i_star) = (T_c/v_0)*( SMC(flu.v_coup_i) + gam * (-SMC(flu.J_NaK_i) - SMC(flu.J_Cl_i) - 2*SMC(flu.J_VOCC_i) - SMC(flu.J_NaCa_i) - SMC(flu.J_K_i) - SMC(flu.J_stretch_i) - SMC(flu.J_KIR_i)));
dy(ind.w_i_star) = (T_c)*( lab * (SMC(flu.Kactivation_i) - state(ind.w_i_star)));
dy(ind.I_i_star) = (T_c/Ca_0)*( SMC(flu.IP3_coup_i) - SMC(flu.J_degrad_i)) ;
dy(ind.K_i_star) = (T_c/K_0)*( -SMC(flu.J_KIR_i) - SMC(flu.J_K_i) + SMC(flu.J_NaK_i)); % uM s-1

% Endothelium cell
dy(ind.Ca_j_star) = (T_c/Ca_0)*( EC(flu.Ca_coup_j) + EC(flu.rho_j) * (EC(flu.J_IP3_j) - EC(flu.J_ERuptake_j) + EC(flu.J_CICR_j) - EC(flu.J_extrusion_j) + EC(flu.J_leak_j) + EC(flu.J_cation_j) + EC(flu.J_0_j) + EC(flu.J_stretch_j)));
dy(ind.s_j_star) = (T_c/Ca_0)*( EC(flu.J_ERuptake_j) - EC(flu.J_CICR_j) - EC(flu.J_leak_j) );
dy(ind.v_j_star) = (T_c/v_0)*( - 1/C_m * ( EC(flu.J_K_j)	+ EC(flu.J_R_j) ) + EC(flu.v_coup_j));
dy(ind.I_j_star) = (T_c/Ca_0)*( EC(flu.IP3_coup_j) + J_PLC - EC(flu.J_degrad_j)) ;
...
dy(ind.R_star)= (T_c/R_0)*( R0pas_r/nu_r *((state(ind.R_star)*R_0)*P_r/SMC(flu.h_r) - E_r * (((state(ind.R_star)*R_0) - R0_r)/R0_r)));
