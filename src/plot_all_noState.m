...
a = zeros(1,44);%25 %35 Farr %43trpv
s = zeros(1,25); %27TRPV
e = zeros(1,17);
f= zeros(1,29); %24
dfdt= zeros(1,31);%24
...
for j = 2:4
    subplot(3,2,j)
    plot(time,DATA(:,stoff+ind.Mp+j-2))
    xlabel('Time')
    ylabel('Fraction [-]')
    title(MCtitle{j})
    hold all
...
subplot(3,2,5)
plot(time,DATA(:,stoff+ind.AMp)+DATA(:,stoff+ind.AM))
xlabel('Time')
ylabel('Fraction [-]')
title(' F_r')
hold all
...
subplot(3,2,6)
plot(time,1e6*DATA(:,stoff+ind.R))
xlabel('Time')
ylabel('Radius in um')
title('Radius')
hold all
...
subplot(4,2,1)
    plot( time,DATA(:,stoff+ind.N_K_k));
title('Astrocyte Potassium concentration')
xlabel('Time [s]')
ylabel('[K] [\muM]')
subplot(4,2,2)
    plot( time,DATA(:,stoff+ind.ck));
title('Astrocyte Calcium concentration')
xlabel('Time [s]')
ylabel('[Ca2+] [\muM]')
subplot(4,2,3)
    plot( time,DATA(:,stoff+ind.ik));
title('Astrocyte IP3 concentration')
xlabel('Time [s]')
ylabel('[IP3] [\muM]')
subplot(4,2,4)
    plot( time,DATA(:,stoff+ind.eetk));
title('Astrocyte EET concentration')
xlabel('Time [s]')
ylabel('[EET] [\muM]')
subplot(4,2,5)
    plot( time,DATA(:,stoff+ind.w_k));
title('Open state of the BK-channel')
xlabel('Time [s]')
ylabel('[K] [\muM]')
subplot(4,2,6)
    plot( time,DATA(:,flu.J_BK_k)./DATA(:,stoff+ind.R_k));
